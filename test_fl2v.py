#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""MiniMax H3 FL2VA 首尾帧插值 —— 通过 ComfyUI API 跑单段视频。

用法: test_fl2v.py <first.png> <last.png> <prompt_file> <seconds> <mp> <steps> <prefix> [seed]
"""
...
FIRST = sys.argv[1]
LAST = sys.argv[2]
PROMPT = open(sys.argv[3], encoding="utf-8").read().strip()
SECONDS = float(sys.argv[4]) if len(sys.argv) > 4 else 5.0
MP = float(sys.argv[5]) if len(sys.argv) > 5 else 0.4
STEPS = int(sys.argv[6]) if len(sys.argv) > 6 else 8
PREFIX = sys.argv[7] if len(sys.argv) > 7 else "video/H3-FL2V"
SEED = int(sys.argv[8]) if len(sys.argv) > 8 else 123456789
...
pid = r["prompt_id"]
print(f"submitted prompt_id={pid} first={FIRST} last={LAST} seconds={SECONDS} mp={MP} steps={STEPS} frames={frames} seed={SEED}", flush=True)
t0 = time.time()
while True:
    time.sleep(5)
    try:
