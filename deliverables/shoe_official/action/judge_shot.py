...
ITEMS = {
    ...
    # ⚠️ R-04 / R-08 拆分（2026-09-18 D 片实测，**判据越界**的典型）：
    #    原 R-04 的 claim 只问「逐字一致」，但 D 片判官答 no 的理由却是**位置**
    #    （"upper-left overlay text 'Air Flows Right Through' sitting on the shoe mesh
    #    rather than the pale backdrop"）。而文字逐字其实**完全正确** ⇒ 判官把
    #    「提示词里的落位要求」当成了「文字声明」的一部分，属**判据颗粒度错误**：
    #    一个 claim 里混了两个事实，任一不合就借 R-04 的名义否决。
    #    ⚠️ 但它揭示的画面问题是**真的**（字压在鞋身上、首帧还被裁掉一个字、部分
    #       落在黑网格上几乎不可读）⇒ 正确做法不是删项，而是按 judge_form.atomic
    #       拆成**两个可观察的原子事实**，各判各的：
    #         R-04 → 只管「逐字是否一致」（拼写/多字/漏字）
    #         R-08 → 只管「落位是否符合声明的落位要求」
    #    ⛔ **编号必须避开 R-07**：`judge_config.yaml` 里 R-07 早已定义为
    #       「结尾镜是否单一全画幅（无网格/分屏）」（官方硬要求，见 REPORT15），
    #       同名不同义会造成审计混乱 ⇒ 新项取下一个空号 = **R-08**。
    "R-04": "Every on-screen text line visible in the frames spells exactly the same words as the text stated in the shot description, with no misspelling, no extra word and no missing word. Answer na if the shot description declares no on-screen text and none is visible.",
    ...
    # ⚠️ R-08 措辞改过一版（2026-09-18 标定）：第一版写成
    #    "The placement … follows the placement requirements stated in the shot
    #     description (on which surface … whether it must keep clear of the product)"
    #    —— 要求判官**先从长句里抽取落位要求、再回画面比对**，是**两步推理**，
    #    实测在已知缺陷（D 片字压鞋）上三票散成 (na,yes,no) ⇒ **不可靠**。
    #    ⇒ 改成**单一可直接观察的事实**：「有没有任何一行字压在鞋身上」。
    #      要求被压成一句二值问题，判官只需看图 ⇒ 与 R-01/R-05 两次措辞修正同源。
    "R-08": "Answer no if any visible on-screen text line is drawn on top of the product itself, meaning any part of the lettering overlaps the sneakers instead of standing on the plain backdrop. Answer na if no on-screen text is visible in the frames, or if the shot description does not state where the text is supposed to stand.",
    # ⚠️ **既有缺口（本次未补，需先标定）**：`judge_config.yaml` 的 **R-07**
    #    「结尾镜是否单一全画幅（无网格/分屏）」是官方硬要求，但**本脚本从未实现**
    #    （旧版只做到 R-06）⇒ 该类失败目前**无判据覆盖**。详见 REPORT28 §六。
    #    ⇒ 所以本脚本的 R 段编号是 R-01…R-06 + **R-08**，**刻意跳过 R-07**。
    # ---- W 段：世界一致性 ----
...
def _tally(vs):
    """多票聚合 → (top, tied)。**并列最高票 ⇒ top=na**（见下方 l2 的踩坑说明）。

    ⛔ 不能写成 `max(cnt, key=cnt.get)`：1:1:1 时它会按 dict 插入顺序返回 "yes"，
       把「判官拿不准」静默洗成「判官确认没问题」。抽成独立函数是为了能单测。
    """
    cnt = {v: sum(1 for r in vs if r["verdict"] == v) for v in ("yes", "no", "na")}
    top = max(cnt, key=lambda v: cnt[v])
    tied = [v for v in ("yes", "no", "na") if cnt[v] == cnt[top]]
    return ("na" if len(tied) > 1 else top), (len(tied) > 1)


def l2(path, shot_prompt, segs, n_frames=6, workers=4, think=False, votes=3, bg="studio"):
    """L2 远端 VLM 判官（**多组抽样投票制**）。

    bg = 故事板声明的背景/景别类型。⚠️ **必须传进 L2**，不能只给 L1：
         L1-11（出框余量）在 macro/outdoor 下已判 na，但 **P-05 是同一个事实的
         VLM 版本** —— 实测 D 片（声明 --bg macro 的满画幅材质特写）L1-11 正确 na，
         L2 P-05 却仍被照判，答 no（2/3 票 "left and top edges of sneakers cropped
         out of frame"）。**这不是片子缺陷，是"按设计发生的构图"被当成缺陷**。
         ⇒ 与 L1-11、与 cuts 同一套逻辑：判官必须区分【按设计发生的】与【凭空出现的】。
         微距/特写镜里主体满画幅 = 设计 ⇒ P-05 直接强制 na，连 VLM 都不必调（省 3 次调用）。
    ...
    ⚠️ 必须【项级并发】：判项天然彼此独立（judge_form.one_question_per_call），
       ...
       18 项 × 3 票 = 54 次调用；tt-5.6-luna 单次 2–15 s（少数 50 s+）/ workers=4
       ⇒ 单镜约 1.5–3 min，仍远小于单镜生成 9.4–11.4 min ⇒ 判官依然"藏在生成后面"（红线不破）。
    投票规则：多数票；三票各不同（1:1:1）⇒ 判 na + confidence=low（交人工，不否决）。
    """
    # 微距/特写镜：满画幅是设计 ⇒ P-05 强制 na（与 L1-11 同源，见 docstring）
    force_na = {"P-05"} if bg == "macro" else set()
    ...
    forced = sorted(i for i in todo if i in force_na)
    todo = [i for i in todo if i not in force_na]
    ...
    # 强制 na 的判项（微距镜的 P-05）：伪造全票 na 灌进 per，报告与打印格式保持统一
    for iid in forced:
        per[iid] = [{"vote": k, "verdict": "na", "frame": None,
                     "region": "forced-na（bg=%s，与 L1-11 同源）" % bg,
                     "confidence": "high", "error": None} for k in range(len(sets))]

    out = []
    for iid in sorted(per):
        vs = per[iid]
        # ⛔ **并列最高票必须判 na，不能判 yes**（2026-09-18 实测修掉的真 bug）
        #    旧写法 `max(cnt, key=...)` 在 1:1:1 时会按 dict **插入顺序**返回第一个键
        #    = "yes" ⇒ **不确定的判项被静默当成通过**（假通过）。
        #    实测：D 片 R-03/R-08 都是 (na,yes,no) 三票各不同，旧写法全报 "yes"，
        #    与设计文档「三票分歧 ⇒ na + confidence=low，交人工，不否决」**正好相反**。
        #    ⚠️ 这一条对 R/W 段尤其致命：R/W 是「任一 no 即否决」，把分歧吞成 yes
        #       等于把"判官其实拿不准"洗成"判官确认没问题"。
        top, tie = _tally(vs)
        cnt = {v: sum(1 for r in vs if r["verdict"] == v) for v in ("yes", "no", "na")}
        ...
        out.append({
            ...
            **({"tie": True} if tie else {}),
        })
        print("   L2 %-5s %-3s  票 %s%s" % (iid, top, out[-1]["votes"],
              "   ⚠️ 三票分歧 → 判 na（交人工）" if tie else ""), flush=True)
    return out
...
def aggregate(res, low_conf_is_veto=False):
    ...
    # ⚠️ 三票分歧项必须显式暴露：判 na 只是"不否决"，不等于"没问题"
    #    —— 判官拿不准的地方，正是最该人工看一眼的地方（实测 D 片两处真缺陷都落在这里）
    tied = [r["id"] for r in RW + P if r.get("tie")]
    if tied:
        warn.append("三票分歧（已判 na，**非**判定为合格）→ 必须人工复核: " + ", ".join(tied))
...
def main():
    ...
    ap.add_argument("--bg", default="studio", choices=["studio", "outdoor", "macro"],
                    help="故事板声明的背景/景别类型，同时决定 L1-11 出框余量与 L2 P-05 是否可用："
                         "studio 平面静态影棚（启用）｜outdoor 户外（L1-11 na）｜"
                         "macro 微距特写（L1-11 + L2 P-05 均 na，主体满画幅是设计不是缺陷）")
    ...
    if not a.dry:
        ...
        r2 = l2(a.video, shot_prompt, segs, n_frames=a.frames, workers=a.workers,
                think=a.think, votes=a.votes, bg=a.bg)
    else:
        print("[L2] 已跳过（--dry）")
